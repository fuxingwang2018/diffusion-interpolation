...
def main(cfg: DictConfig) -> None:
    ...
    dm.prepare_data()
    dm.setup(cfg.split)
    loader = dm.test_dataloader() if cfg.split == "test"  else dm.val_dataloader() if cfg.split == "val" else dm.train_dataloader()
    first = loader.dataset[0]
    last = loader.dataset[-1]

    print( f" cfg.val len {len(loader.dataset)}")
    print( f"first meta: {first[-1]}")
    print( f"last meta: {last[-1]}")
    print( f"first shape: {first[0].shape}")
